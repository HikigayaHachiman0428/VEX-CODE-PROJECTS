...
//外层函数
  ...
  //赛季特别方法
   ...
   int back_claw(bool up){timer reg_time; reg_time.reset();int time_lim = 1000; int direct = up?1:-1; int degree_lim = up? 0:-880 ;while (reg_time.time()<time_lim && BackClaw.position(degrees)>= degree_lim) {BackClaw.setVelocity(direct*100, percent);BackClaw.spin(fwd);} BackClaw.stop(hold); return 0;}
  //task
    int this_init;
