...
//const系数
  //Go 参数
    ...
    const double UNIT_RATIO=0.5;//to change the unit-----1000represent one block(60cm)  
  ...
  //转弯
    //90
      ...
      const int PIDTURN_STOPTIME=1000;
      const double PIDTURN_ERROR_ALLOWENCE=1;

  
    //45
      const double RKP_SMALL=0.08;
      const double RKI_SMALL=0.89;

      const double RKP_SMALL_FAST=0.7;
      const double RKI_SMALL_FAST=0.4;

      const int SMALL_PIDTURN_HIGHSPEED=35;
      const int SMALL_PIDTURN_MIDDLESPEED=20;

      const double SMALL_PIDTURN_HIGHSPEED_INCREMENT=0.7;
      const double SMALL_PIDTURN_MIDDLESPEED_INCREMENT=0.5;
      const double SMALL_PIDTURN_LOWSPEED_INCREMENT=1;

      const int SMALL_PIDTURN_STOPTIME=1500;
      const double SMALL_PIDTURN_ERROR_ALLOWENCE=2;
    ...
    //135
      ...
      const int BIG_PIDTURN_STOPTIME=1500;
...
//外层函数
  ...
  //turn
    ...
    void turnto_logic(double velocity,double angle){
      ...
      while(1){
        ...
        set_V(speed,-speed);
        if(abs(Inert_Constant.heading()-turn_to_angle)<this_ERROR_ALLOWENCE){break;}
        else continue;
        if(autoturn_time_limit.time()>this_STOPTIME)break;
        wait(10,msec);
      }
      set_torque(100);
      set_V(0);
  ...
  //赛季特别方法
    ...
    int fast_down_clawclose(int inti){DownClaw.setPosition(inti, degrees);timer reg_time; reg_time.reset();while (reg_time.time()<2000 && (!(135<DownClaw.position(degrees)&&DownClaw.position(degrees)<145))){DownClaw.setVelocity(100, percent);DownClaw.spin(fwd);}DownClaw.setVelocity(0, percent);return 0;}

   int back_claw(bool up, int target){timer reg_time; reg_time.reset();int time_lim = 500; int direct = up?1:-1; while (reg_time.time()<time_lim) if(up)while (BackClaw.position(degrees)<= target){BackClaw.setVelocity(direct*70, percent);BackClaw.spin(fwd);} else while (BackClaw.position(degrees)>= target){;BackClaw.setVelocity(direct*100, percent);BackClaw.spin(fwd);} BackClaw.stop(brake); return 0;}

   int back_claw_to_bottom(){back_claw(0,-900); return 0;}
   int back_claw_to_top(){back_claw(1,0);return 0;}
   int back_claw_take(){back_claw(1,-300);return 0;}
  //task
  bool back_claw_holding;
  int back_keep_holding(){
    while (back_claw_holding){
      BackClaw.setMaxTorque(70, percent);
      BackClaw.setVelocity(10,percent);
    }
    return 0;
  }
  ...
  int front_keep_holding(){
    while (front_claw_holding){
      DownClaw.setMaxTorque(70, percent);
      DownClaw.setVelocity(10,percent);
    }
    return 0;
  }

 int this_init;
int task_up(){up_lift(0);return 0;}
int task_down(){wait(550);down_lift(650);return 0;}
int task_close(){wait(1400);fast_up_clawclose();return 0;}
void Auto(void) {  
  timer auto_timer;
  bool COMPILING=0;
  set_encoder();
  if(COMPILING){
    ...
    while(true){
      if (Controller1.ButtonUp.pressing()){
        ...
        task t(back_keep_holding);
        wait(100);
        back_claw_take();
      }
      if (Controller1.ButtonDown.pressing()){
        back_claw_holding = false;
        back_claw_to_bottom();
    }
  ...
  task t2(back_claw_to_bottom);
  back_claw_holding = false;
  task t1(back_keep_holding);
  down_lift(5);
  fast_down_clawclose();
  task t4(front_keep_holding);
  ...
  wait(100);
  back_claw_holding = false;
  BackClaw.setPosition(-300,degrees);
  back_claw_to_bottom();
  Go(100,500);
  task t3(back_claw_to_top);
  turnto(100,245);
  wait(100);
  ...
  front_claw_holding = false;
  fast_down_clawopen();

  t1.stop(); t2.stop();t4.stop(); t3.stop();T1.stop();
