...
//外层函数
  //基础模块
    ...
    double av_CLEncoder   (){return (FrontClaw.position(degrees)+BackClaw.position(degrees))/2;}
  ...
  //赛季特别方法
    ...
    // int fast_down_clawopen(){timer reg_time; reg_time.reset();while(reg_time.time()<200 &&(!(0<DownClaw.position(degrees)&& DownClaw.position(degrees)<5))){DownClaw.setVelocity(-100, percent);DownClaw.spin(fwd);}DownClaw.setVelocity(0, percent);return 0;}
    // int fast_up_clawclose(){timer reg_time; reg_time.reset();while(reg_time.time()<500 &&(!(745<UpClaw.position(degrees)&&UpClaw.position(degrees)< 755))){UpClaw.setVelocity(100, percent);UpClaw.spin(fwd);}UpClaw.setVelocity(0, percent);return 0;}
    // int fast_down_clawclose(){timer reg_time; reg_time.reset();while (reg_time.time()<100 && (!(135<DownClaw.position(degrees)&&DownClaw.position(degrees)<145))){DownClaw.setVelocity(100, percent);DownClaw.spin(fwd);}DownClaw.setVelocity(0, percent);return 0;}
    // int fast_down_clawclose(int inti){DownClaw.setPosition(inti, degrees);timer reg_time; reg_time.reset();while (reg_time.time()<500 && (!(143<DownClaw.position(degrees)&&DownClaw.position(degrees)<147))){DownClaw.setVelocity(100, percent);DownClaw.spin(fwd);}DownClaw.setVelocity(0, percent);return 0;}
