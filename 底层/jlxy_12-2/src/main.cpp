...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if (!L1Pressed && L1) {
      if (!mogoFState && getRLEncoder() > RLWaitAngle + 15)
        RLState = RLState >= 2 ? 1 : RLState;
      else
        mogoFState = !mogoFState;
    }
    if (!L2Pressed && L2) {
      if (!mogoBState && getRLEncoder() < RLWaitAngle - 15)
        RLState = RLState == 0 ? 1 : RLState;
      else
        mogoBState = !mogoBState;
    }
    if (getMLEncoderF() < 0)
      resetMLEncoderF();
    if (getMLEncoderB() > 0)
    ...
    if (mogoFState) {
      setFrontLock(true);
      if (getMLEncoderF() < mogoFTakeAngle - 3) {
        liftMogoF(L1 ? 100 : 127);
      } else
        liftMogoF(15);
    } else {
      if (getMLEncoderF() > 3) {
    ...
    if (mogoBState) {
      ...
      if (getMLEncoderB() > mogoBTakeAngle)
        liftMogoB(L2 ? -60 : -127);
      else
        liftMogoB(-5);
    }
    ...
    Brain.Screen.print("                           ");
    Brain.Screen.setCursor(7, 1);
    Brain.Screen.print("KickStandState: %d", KickStandState);
