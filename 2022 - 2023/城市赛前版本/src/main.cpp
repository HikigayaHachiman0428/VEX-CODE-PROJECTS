...
void usercontrol(void) {
    ...
    manual = 1; AutoCataInterrupt = 0; lck = 1; lckReset = 1; ch_state = 2;  
    while (1) {
      // one();
      // break;
      if(LEFT && !lastLEFT) autoRoutine = autoRoutine == 0 ? 3 : autoRoutine-1;
      if(RIGHT && !lastRIGHT) autoRoutine = autoRoutine == 3 ? 0 : autoRoutine+1;
        if(manual){
            if((L1&&L2&&!lastL1)||(L1&&L2&&!lastL2)){manual = 0; autoCata = 1;lck = 0; }
            if((!L1&&(R1||R2)&&lastL1)||(L1&&!R1&&lastR1)||(L1&&!R2&&lastR2)){lck = 1; lckReset = 1;}
            if(L1&&(R2||R1)){lck = 0;}
            if(!lck){cata(100*L1*(R2-R1));}
            
        }else{
            if((L1&&L2&&!lastL1)||(L1&&L2&&!lastL2)){AutoCataInterrupt = 1;}
        }
        ...
        lastL1 = L1; lastL2 = L2; lastR1 = R1;
        lastR2 = R2; lastLEFT = LEFT; lastRIGHT = RIGHT;
        delay(10);
    }
}
